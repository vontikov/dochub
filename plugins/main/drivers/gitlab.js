...
const api = {
    ...
    compare: async(from, to) => {
        ...
        console.info(`COMPARE from ${from} to ${to}`, result);
...
const driver = {
    ...
    // Проверяет паторизацию, есил она не пройдена, отправляет в gitlab
    checkAuth() {
    ...
    logout() {
        ...
        OAuthCode = null;
        cookie.delete('gitlab-token-access');
        cookie.delete('gitlab-token-refresh');
        if (status.isLogined) {
            this.onChangeStatus();
    ...
    bootstrap(context) {
        ...
        this.config.appSecret = context.env.VUE_APP_DOCHUB_CLIENT_SECRET;
