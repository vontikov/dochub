        ...
        modes: [
          {
            name: 'Отключено',
            id: 'disable',
            hint: 'Инткграция отключена.'
          },
          {
            name: 'DocHub Registry',
            id: 'registry',
            hint: 'Сервис авторизации DocHub для доступа к облачным репозиториям https://gitlab.com/. Не требует настройки.'
          },
          {
            name: 'Персональный токен',
            id: 'personal_token',
            hint: 'Режим доступа к произвольному серверу с использованием персонального токена. Требуется выпуск токена и настройка разрешений сервера. Введенная информация НЕ ХРАНИТСЯ на серверах DocHub. Она сохранятся в локальном хранилище вашего браузера.'
          }
        ]
      };
    },
    ...
    methods: {
      ...
      async doCheckIntegration() {
        ...
        switch (result?.response?.status) {
          ...
          case 503: this.checkError = 'Сервер временно недоступен. Повторите попытку похже.'; break;
          default:
            if (Number.parseInt(result.response?.status) >= 500) 
              this.checkError = 'Возникла непредвиденная ошибка на сервере.';
            else 
              this.checkError = 'Вероятно в параметрах интеграции ошибка. Проверьте введенные значения параметров и поторите попытку';
        }
        return false;
      },
      validate() {
