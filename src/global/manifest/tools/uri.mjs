...
export default function(config) {
    ...
    this.isExternalURI = (uri) => {
        return !uri.startsWith(window?.origin) && this.isURL(uri);
    };
    this.makeURIByBaseURI = (uri, baseURI) => {
        ...
        try {
            ...
            const driver = DocHub?.protocols.get(protocol.split(':')[0]);
            if (driver) {
                result = driver.resolveURL(baseURI, uri);
            } else if (protocol === 'gitlab:' || protocol === 'bitbucket:') { // Устаревшая реализация резолва нужно зарефачить для backend
